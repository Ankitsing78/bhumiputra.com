(function(){
  ...
  function renderCartDrawer(open){
    ...
    html += '<div class="summary-row total-row"><span>Total</span><strong>'+formatCurrency(t.total)+'</strong></div>';
    html += '<div class="drawer-actions"><button id="bp-checkout" class="btn-primary">Checkout</button><button id="bp-continue" class="btn-secondary">Continue</button><button id="bp-clear" class="btn-tertiary">Clear</button></div>';
    html += '</div>';
    root.innerHTML = html;
  ...
  function getQueryParam(name){ try{ return new URLSearchParams(location.search).get(name); }catch(e){ return null; } }

  // product helpers
  function addCurrentProductToCart(){ const titleEl = document.querySelector('.detail-title, .p-title, .product-name, .p-name, .product-title'); const title = titleEl? titleEl.textContent.trim() : 'Product'; const priceEl = document.querySelector('.price-large, .price-factory, .price-main, .price'); const price = priceEl? priceEl.textContent.trim() : ''; addToCart({id:slugify(title),name:title,price:price,qty:1}); }
  function addCheckoutProductFromQuery(){
    const lookup = {
      ...
      'co-driver': {id:'co-driver',name:'High-Back Co-Driver Seat — Folding',price:'₹2,900',qty:1},
      'silencer-oval': {id:'silencer-oval',name:'Oval Silencer — Matte Black Finish',price:'₹2,100',qty:1}
    };
    var addKey = getQueryParam('add') || getQueryParam('checkout'); if(!addKey) return;
    var item = lookup[addKey]; if(!item) return;
    var idx = getCart().findIndex(i=>i.id===item.id);
    if(idx === -1){ addToCart(item); }
  }
  ...
  function attachFilterUI(){
    ...
    if(!filters) return;
    if(!document.getElementById('bp-filter-toggle')){
      var btn = document.createElement('button');
      btn.id = 'bp-filter-toggle';
      btn.type = 'button';
      ...
      btn.addEventListener('click', function(){ filters.classList.toggle('open'); btn.classList.toggle('active'); });
    }
    document.querySelectorAll('.filter-option').forEach(function(opt){
      if(opt.dataset.bpFilterAttached) return;
      opt.dataset.bpFilterAttached = '1';
      opt.addEventListener('click', function(){ this.classList.toggle('selected'); });
    });
    updateFilterToggleDisplay();
    window.addEventListener('resize', updateFilterToggleDisplay);
  }

  function updateFilterToggleDisplay(){
    var filters = document.querySelector('.filters');
    var btn = document.getElementById('bp-filter-toggle');
  ...
  function attachHandlers(){
    ...
    document.querySelectorAll('.sugg-pill').forEach(function(pill){
      if(pill.dataset.bpSuggAttached) return;
      pill.dataset.bpSuggAttached = '1';
  ...
  function placeOrder(){
    try{
      ...
      var phone = '';
      try{ var userData = getStoredUser() || {}; phone = (userData && userData.phone) ? userData.phone : (userData && userData.id ? userData.id : ''); }catch(e){ phone = ''; }
      var order = {
        id: 'BPT-'+Math.floor(100000+Math.random()*899999),
        created: new Date().toISOString(),
        customer: { name: addr.name||user.name||'Guest', phone: phone },
        address: addr,
        items: items,
        totals: totals,
  ...
  document.addEventListener('DOMContentLoaded', function(){
    try{
      ...
      if(header && !document.querySelector('.cart-toggle') && !document.querySelector('#cart-btn-top') && !header.querySelector('.cart-util')){
  ...
  // inject user/profile icon in header only when there is no auth button already
  document.addEventListener('DOMContentLoaded', function(){
    try{
      var header = document.querySelector('header') || document.querySelector('.site-header') || document.querySelector('.header');
      if(!header || document.querySelector('#auth-btn-top') || document.querySelector('#user-btn-top') || document.querySelector('.user-toggle')) return;
